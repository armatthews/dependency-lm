...
class BottomUpDepLM:
  def __init__(self, pc, action_vocab, word_vocab_size, rel_vocab_size, hidden_dim, labelled=True):
    layers = 1
  ...
  def build_graph(self, sent):
    ...
    for action, subtype in sent:
      ...
      if action_str == 'shift':
        word_emb = dy.lookup(self.word_embs, subtype)
        self.stack.append(word_emb)
      elif action_str == 'right':
        assert len(self.stack) >= 2
        head = self.stack[-1]
        child = self.stack[-2]
        combined = self.combine(head, child, 'right')
        self.stack.append(combined)
...
def main():
  ...
  model = BottomUpDepLM(pc, action_vocab, len(terminal_vocab), len(rel_vocab), 64, False)
